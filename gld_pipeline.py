...
def build_features(df):
    ...
    f['vol60']       = log_c.diff().rolling(60).std()
    f['day_of_week'] = df.index.dayofweek / 4
    f['month']       = pd.to_datetime(df.index).month / 12
    return f.dropna()

feature_cols = ['mom5','mom20','mom60','overnight','intraday',
                'vol20','vol60','day_of_week','month']

# ── Main Pipeline ─────────────────────────────────────────────────────────────

print("Loading GLD data...")
df = pd.read_parquet('GLD_daily.pq')
...
if pval < 0.05:
    ...
    print("RESULT: Marginal edge — add more features")
elif pval < 0.20:
    print("RESULT: Promising — keep iterating")
else:
...
if permuted_pfs:
    ...
    plt.axvline(real_pf, color='red', label=f'GLD Strategy (PF={real_pf:.2f})')
    plt.xlabel("Profit Factor")
    plt.title(f"GLD Triple Barrier MCPT - P-Value: {pval:.3f}")
    plt.legend()
    plt.savefig('gld_mcpt.png')
    plt.show()